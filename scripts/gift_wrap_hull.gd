...
func GenerateConvexHull(shape_vertices: PackedVector3Array) -> Array[PackedVector3Array]:
	...
	for i in range(3):
		var current_best_vert := Vector3.INF # start at +infinity
		for vert in vertices:
			if vert in lowest_verts: continue # skip already found verts
			if vert.z < current_best_vert.z: # TEST: (fixed?) fix when multiple points have the same z
				current_best_vert = vert
			elif vert.z == current_best_vert.z and \
					vert.y < current_best_vert.y:
				current_best_vert = vert
			elif vert.z == current_best_vert.z and \
					vert.y == current_best_vert.y and \
					vert.x < current_best_vert.x:
				current_best_vert = vert
	...
	# TODO: add all points in this plane to the hull
